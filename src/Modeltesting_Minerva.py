...
for technique in prompting_techniques:
    ...
    for idx in range(1):  # Run for first 2 records as an example
        ...
        try:
            minerva_output = get_model_output(prompt, model_name='minerva', technique=technique)  # Pass technique for accurate response extraction
            print(f"Minerva output for record {idx + 1}: {minerva_output}")
